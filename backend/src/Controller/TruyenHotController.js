// Import model TruyenHot từ thư mục Model
...
const getTruyenHotController = async (req, res) => {
    try {
        ...
        const { count, rows } = await TruyenHot.findAndCountAll({
            limit,
            offset,
        ...
        return res.status(200).json({ TruyenHotController: rows, total: count });
    } catch (error) {
