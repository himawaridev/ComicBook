"use strict";

const { Model, DataTypes } = require("sequelize");

module.exports = (sequelize) => {

    class TruyenMoiCapNhat extends Model {
        // Khai báo class `TruyenMoiCapNhat` kế thừa từ Sequelize `Model`.

        /**
         * Helper method for defining associations.
         * This method is not a part of Sequelize lifecycle.
         * The `models/index` file will call this method automatically.
         */
        static associate(models) {
            // Định nghĩa các mối quan hệ (association) với các model khác tại đây nếu cần.
        }
    }

    TruyenMoiCapNhat.init(
        {
            Slug: {
                type: DataTypes.STRING,
                allowNull: false,
                defaultValue: '', // 🔹 Thêm giá trị mặc định
                validate: {
                    notNull: { msg: "ImageLinks is required" },
                },
            },
            ImageLinks: {
                type: DataTypes.STRING,
                allowNull: false,
                validate: {
                    notNull: { msg: "ImageLinks is required" },
                },
            },
            Title: {
                type: DataTypes.STRING,
                allowNull: false,
                validate: {
                    notNull: { msg: "TitleIntro is required" },
                },
            },
            LinkComic: {
                type: DataTypes.STRING,
                allowNull: false,
                validate: {
                    notNull: { msg: "TitleIntro is required" },
                },
            },
            Author: {
                type: DataTypes.STRING,
                allowNull: false,
                validate: {
                    notNull: { msg: "NameComic is required" },
                },
            },
            Chapters: {
                type: DataTypes.STRING,
                allowNull: false,
                validate: {
                    notNull: { msg: "Rating is required" },
                },
            },
            // StatusCrawler: Number // 0: Đang cập nhật, 1: Full ?? fixx this heres
        },
        {
            sequelize,
            modelName: "TruyenMoiCapNhat",
            freezeTableName: true,
            tableName: "TruyenMoiCapNhat",
            timestamps: true,
        }
    );

    return TruyenMoiCapNhat;
}


